...
static const sd_mbr_command_t mbr_command_init = {
    .command = SD_MBR_COMMAND_INIT_SD,
};

void jump_to_app(uint32_t *app_isr) {
#if DEBUG
    uart_disable();
#endif
    ...
    // Let the SoftDevice know it should use the app's vector table from
    // now on instead of ours.
    sd_softdevice_vector_table_base_set((uint32_t)app_isr);

    // The address `app_addr` points to the ISR vector of the app.
    ...
    __asm__ __volatile__(
            "ldr r1, [%[app_isr], #0]\n" // load end of stack (_estack)
            "ldr r2, [%[app_isr], #4]\n" // load Reset_Handler
            "mov sp, r1              \n" // set stack pointer to beginning
            "bx  r2                  \n" // jump to app (never to return)
            :
            : [app_isr]"r" (app_isr));
...
void _start(void) {
    ...
    // Initialize the MBR. I don't know why this is necessary, but
    // otherwise the SoftDevice won't work.
    LOG("init mbr");
    sd_mbr_command((sd_mbr_command_t*)&mbr_command_init);
    ...
    // matters when the application requested to go to the bootloader by
    // setting this register (it defaults to 0).
    NRF_POWER->GPREGRET = 0;

    // Use the booloader's interrupt table. Apparently this is not done in
    // the MBR.
    LOG("set sd vector table");
...
void handle_command(uint16_t data_len, ble_command_t *cmd) {
    ...
    if (cmd->any.command == COMMAND_RESET) {
        ...
        jump_to_app((uint32_t*)APPLICATION_START_ADDR);
#endif
    } else {
        LOG("command: ???");
    }
